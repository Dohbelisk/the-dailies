...
export class FriendsService {
  ...
  async sendFriendRequest(senderId: string, receiverId: string): Promise<FriendRequest> {
    ...
    const existingFriendship = await this.friendModel.findOne({
      $or: [
        { userId: new Types.ObjectId(senderId), friendId: new Types.ObjectId(receiverId) },
        { userId: new Types.ObjectId(receiverId), friendId: new Types.ObjectId(senderId) },
      ],
    }).exec();
  ...
  async sendFriendRequestByCode(senderId: string, friendCode: string): Promise<FriendRequest> {
