import { Controller, Post, Get, Delete, Body, Param, Query, UseGuards, Request, NotFoundException } from '@nestjs/common';
...
export class FriendsController {
  ...
  @Post('request')
  async sendFriendRequest(@Request() req, @Body() body: { receiverId: string }) {
    return this.friendsService.sendFriendRequest(req.user.userId, body.receiverId);
  }

  @Post('request/code')
  async sendFriendRequestByCode(@Request() req, @Body() body: { friendCode: string }) {
    return this.friendsService.sendFriendRequestByCode(req.user.userId, body.friendCode);
  }

  @Post('request/username')
  async sendFriendRequestByUsername(@Request() req, @Body() body: { username: string }) {
    ...
    return this.friendsService.sendFriendRequest(req.user.userId, user._id.toString());
  }
